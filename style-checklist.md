...
## 1. Route integrity
- The request resolves to one legal `document_type` or one legal `surface + document_shape` route in `artifact-presets.json`.
...
- All colors come from `design-tokens.json`.
