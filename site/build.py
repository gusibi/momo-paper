...
def generate_sitemap():
    ...
    for _, url_path, page_id in PAGES:
        page_url = build_page_url(url_path)
        priority = "1.0" if page_id == "index" else "0.8" if page_id in ("guide", "components", "faq") else "0.6" if page_id == "demo" else "0.5"
        freq = "weekly" if page_id in ("index", "guide", "components", "faq") else "monthly"
