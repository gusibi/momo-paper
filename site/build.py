...
PAGE_META = {
    "index": {
        "description": "Momo Paper 是面向文档与视觉叙事的开源路由式设计系统，提供 15 种文档类型、5 种可编程图表、14 种图示原语和 30 个 HTML 模板。Markdown DSL 声明内容，引擎渲染设计精良的 HTML。",
...
Momo Paper 是一个开源的文档生成引擎，输入 Markdown DSL，输出排版精良、打印就绪的 HTML 文档。核心思想是「路由式设计系统」——你声明 document_type 与结构化块（:::block），引擎渲染设计精良的 HTML。
...
- **5 种可编程图表**: bar, line, donut, candlestick, waterfall — DSL 声明 chart 块，引擎自动渲染
- **14 种图示原语**: 架构图、流程图、象限图、状态机、泳道图、时间线、树状图、分层图、维恩图等
- **双主题系统**: Momo Paper 暖调主题与 Vercel 极简主题，一键切换
- **设计令牌系统**: CSS 自定义属性集中定义颜色、字体、间距
- **AI Agent 集成**: 支持 Claude Code Skill 和 CLI 调用
