...
pip install -e .
momo schema list
momo validate examples/research-summary.md --json
momo render examples/research-summary.md -o dist/research-summary.html
```
...
momo validate examples/landing.md
momo render  examples/landing.md -o dist/landing.html
```
...
# 2. 校验；正式模板进入 strict mode
momo validate report.md --json
```
...
Agent 读 `code`、`line`、`block` 和 `field` 定位问题，修复全部错误后重新校验。正式模板 unique match 时使用 strict mode；未注册类型进入 free mode 并给出 warning，但 render 仍保留未知内容。通过后渲染：

```bash
# 2. 渲染为单文件 HTML（CSS 内联）
momo render report.md -o report.html
```
