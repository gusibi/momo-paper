...
`document_type` 是 frontmatter 中的语义标签，出现在页头并引导 Skill 选择模板；引擎本身不按类型做结构校验。下方组件在所有 `document_type` 中通用。
...
body: document_type 是语义标签，出现在页头并引导 Skill 选模板。引擎不按类型做结构校验——任何类型都可使用全部组件。
:::
