...
#   momo_dsl/            engine: parser, renderer, cli, schemas, styles
...
#   momo-paper-skill/references/templates/
...
# 1. Generate per-template Skill references from machine schemas.
python3 "$ROOT/scripts/generate-schema-references.py"

# 2. Engine code + bundled schemas/CSS -> runtime/momo_dsl (exact mirror).
...
echo "generated: schemas        -> $SKILL/references/templates/"
